...
import {
    ...
    removeSerial,
    resetSerial
} from '@/redux/slice/assistant/serialSlice';
...
const SerialList = () => {
    ...
    useEffect(() => {
        const fetchDoctorDepartmentSchedule = async () => {
            try {
                const response = await axios.get(`${config.api}/assistant/doctor-department-schedule`, {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.get(`${config.api}/assistant/serial/all`, {
                params: data,
                headers: {
    ...
    const reduxStoreSerial = useSelector(state => state.assistant_serials.data);
    const totalItems = useSelector(state => state.assistant_serials.totalItems);
    const fetchedItems = useSelector(state => state.assistant_serials.fetchedItems);
    ...
    const handleStatusChange = async (event, id) => {
        try {
            ...
            const response = await axios.post(`${config.api}/assistant/serial/update/status`, data, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
    ...
    const handelDelete = async () => {
        try {
            await axios.delete(`${config.api}/assistant/serial/${deleteItem?.id} `, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleInvoiceDownload = async (data) => {
        ...
        try {
            ...
            const response = await axios.get(`${config.api}/assistant/serial/${data.id}/serial-number`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
                            ...
                            reduxStoreSerial.length > 0 ?
                                ...
                                (
                                    found ? <div className={styles.notFound}>
                                        <h6 className='fw-bold text-uppercase'>no serial found</h6>
                                    </div> : null
                                )
                        }
                    </div>
